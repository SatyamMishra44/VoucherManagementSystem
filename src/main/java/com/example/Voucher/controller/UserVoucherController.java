...
public class UserVoucherController {
        ...
        @Operation(summary = "Step 4 (User): View Eligible Voucher Templates", description = "See templates that are currently active and within valid date range.")
        ...
        @Operation(summary = "Step 5 (User): Purchase Voucher", description = "Buy a voucher using template code and quantity. System creates your personal voucher balance.")
        public ResponseEntity<UserVoucherResponseDto> purchaseVoucher(
                ...
                UserVoucher userVoucher = userVoucherService.purchaseVoucher(userId, request.getVoucherCode(),
                                request.getQuantity(), requestId);
                return ResponseEntity.ok(UserVoucherResponseDto.fromEntity(userVoucher));
        }

        @PostMapping("/redeem")
        @PreAuthorize("hasAuthority(@roleProperties.getUser())")
        @Operation(summary = "Step 6 (User): Redeem Voucher Balance", description = "Apply your voucher balance to a bill amount. Supports partial redemption and records an audit entry.")
        ...
        @GetMapping("/mine")
        @PreAuthorize("hasAuthority(@roleProperties.getUser())")
        @Operation(summary = "User: View My Purchased Vouchers", description = "See your purchased vouchers with status and remaining balance.")
        ...
        @PreAuthorize("hasAuthority(@roleProperties.getUser())")
        @Operation(summary = "Step 7 (User): View My Redemption History", description = "Get complete redemption records for audit: redeemed amount, remaining balance, date, and bill reference.")
        ...
        @Operation(summary = "Admin: Filter Issued User Vouchers", description = "Filter issued vouchers by amount, redemption state, issued/expiry date range, status, and assigned user.")
